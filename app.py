...
app = Flask(__name__)
app.secret_key = "applex_super_stable_final"
...
<head>
    ...
    <style>
        ...
        .header { background: white; padding: 15px; display: flex; justify-content: space-between; box-shadow: 0 2px 5px rgba(0,0,0,0.1); position: sticky; top: 0; z-index: 10; }
        ...
        .card { background: linear-gradient(135deg, #667eea, #764ba2); color: white; padding: 20px; border-radius: 15px; margin-top: 15px; text-align: center; }
        .btn { background: white; color: #764ba2; border: none; padding: 10px 20px; border-radius: 20px; font-weight: bold; margin-top: 10px; width: 100%; cursor: pointer; }
        ...
        .nav-a { text-align: center; color: #666; text-decoration: none; font-size: 12px; }
...
{% if page == 'dashboard' %}
    ...
    <div class="container">
        ...
        <div class="grid">
            ...
            <div class="box"><b>৳{{ user['job_bal'] }}</b><span>Job Wallet</span></div>
            <div class="box"><b>৳{{ user['refer_bal'] }}</b><span>Refer Bonus</span></div>
            <div class="box"><b>৳{{ user['total_withdraw'] }}</b><span>Withdraw</span></div>
        </div>
        <div class="card">
            <h4 style="margin:0;">Premium Level</h4>
            <button class="btn" onclick="openDep()">Upgrade Now</button>
        </div>
    </div>
    <div id="depM" class="modal">
        <div class="m-content">
            ...
            <p style="color:red; font-size:12px;">Bkash/Nagad/Rocket (Personal)</p>
    ...
    <div class="nav">
        ...
        <div class="nav-a" onclick="openDep()"><i class="fas fa-wallet"></i><br>Deposit</div>
...
{% elif page == 'login' %}
    <div style="max-width:350px; margin:100px auto; padding:25px; background:white; border-radius:15px; box-shadow:0 5px 15px rgba(0,0,0,0.1); text-align:center;">
        <h2>Login Account</h2>
        <form method="POST">
            <input type="tel" name="phone" placeholder="Phone Number" style="width:100%; padding:12px; margin:10px 0; border-radius:8px; border:1px solid #ddd; box-sizing:border-box;" required>
            <input type="password" name="pass" placeholder="Password" style="width:100%; padding:12px; margin:10px 0; border-radius:8px; border:1px solid #ddd; box-sizing:border-box;" required>
            <button type="submit" style="width:100%; padding:12px; background:var(--blue); color:white; border:none; border-radius:8px; font-weight:bold;">Login</button>
        </form>
    </div>
{% endif %}
</body>
